...
const ManagePickDrop = () => {
    ...
    const Get_Registered_PickDrop = async () => {
        await fetch('/registered-pickdrop', { method: 'GET' }).then((res) => {
            // console.log(res);
            res.json().then((data) => {
                // console.log(data);
                setPickDrop(data);
    ...
    const onBanAccount = async (id) => {
        await fetch(`/registered-pickdrop/${id}`, {
            method: 'POST',
        }).then((res) => {
            res.json().then((data) => {
                // console.log(data);
    ...
    //Showing data
    const Show_Registered_PickDrop = () => {
        
        return (
            <>
            <Row className="pd-data-row">
                {
                    PickDrop.map((value, index) => {
                        return (
                            <div key={index}>
                            <Col xs={12} sm={6} md={4} lg={10} className="pd-each-data">
                                <Col>
                                <h6>P&D _id : {value._id}</h6>
                                ...
                                <h6>Charges : {value.charges}</h6>
                                <h6>Vehicle number# : {value.vehicle_no}</h6>
                                </Col>
                                <Col>
                                <Button className="pd-data-btn" variant='danger' onClick={() => { onBanAccount(value._id) }}>Ban Account</Button>
                                </Col>
        ...
        if (AdminDashboardValidationState === 'true') {
            ...
            return (
                <>
                    ...
                    <Link to='/manage-pickdrops'><Button variant='outline-info' active block className="Admin-Dashboard-buttons" >Pick & Drops</Button></Link>
