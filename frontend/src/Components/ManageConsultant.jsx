...
import '../css_files/mconsultant.css';
...
const ManageConsultants = () => {
    ...
    const Get_Registered_Consultants = async () => {
        await fetch('/registered-consultant', { method: 'GET' }).then((res) => {
            // console.log(res);
            res.json().then((data) => {
                // console.log(data);
                setConsultants(data);
    ...
    const onBanAccount = async (id) => {
        await fetch(`/registered-consultant/${id}`, {
            method: 'POST',
        }).then((res) => {
            res.json().then((data) => {
                // console.log(data);
    ...
    //Showing data
    const Show_Registered_Consultants = () => {

        return (
            <>
            <Row className="c-data-row">
                {
                    Consultants.map((value, index) => {
                        return (
                            <div key={index}>
                            <Col xs={12} sm={6} md={4} lg={10} className="c-each-data">
                                <Col>
                                <h6>Consultant _id : {value._id}</h6>
                                ...
                                <Col>
                                <Button className="c-data-btn" variant='danger' onClick={() => { onBanAccount(value._id) }}>Ban Account</Button>
                                </Col>
        ...
        if (AdminDashboardValidationState === 'true') {
            ...
            return (
                <>
                    ...
                    <Link to='/manage-consultants'><Button variant='outline-info' active block className="Admin-Dashboard-buttons" >Consultants</Button></Link>
